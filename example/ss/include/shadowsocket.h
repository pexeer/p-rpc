...
public:
    ...
    int try_to_decrypt() {
        std::unique_ptr<p::rpc::ss::StreamDecrypt> ptr;
        for (int i = 0; ; ++i) {
            char buf[kMaxHeaderLen];
            int len = buf_.copy_front(buf, kMaxHeaderLen);
            ptr.reset(p::rpc::ss::StreamDecrypt::get_one(i));
            if (!ptr) {
                // all method failed
                return -1;
            }

            ptr->init_key(g_passwd);
            int used = ptr->init_iv(buf, len);
            if (used <= 0) {
                continue;
            }
            LOG_DEBUG << this << " ShadowSocket buf=" << buf_.size() << ",used=" << used;
    ...
    int connect_remote() {
        ...
        if (ret > 0) {
            delete remote_;
            remote_ = nullptr;
            LOG_WARN << this << "connect retry later ";
            return -2;
        }

        LOG_DEBUG << this << "ShadowSocket RemoteSocket=" << remote_ << " doing connecting";
